...
Customers own their data and have control over where that data will be located. The customer specifies the location where their production and development instances reside. They also specify which location will be used for the Adobe Commerce Reporting environment in conjunction with Commerce, and if the  Adobe Commerce Reporting application has access to personally identifiable information or not. Production instances can be located in most AWS regions, while development and Adobe Commerce Reporting environments can be found in either the United States or in the European Union at this time.
