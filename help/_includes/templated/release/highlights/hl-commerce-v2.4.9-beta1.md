...
Adobe Commerce 2.4.9 is fully compatible with OpenSearch 3.x. This update enables merchants to benefit from improved performance, security, and long-term support while maintaining backward compatibility with OpenSearch 2.x.
...
#### Investigate the LTS symfony dependencies in magento2-functional-testing-framework

As part of ongoing platform health work under the 2.4.9 platform component upgrades, the Magento Functional Testing Framework (MFTF) has been updated to use the latest Symfony LTS dependencies (including symfony/config) required by web-token/jwt-framework. This resolves prior dependency conflicts and ensures a stable, supported stack for functional testing.
...
Adobe Commerce 2.4.9 provides support for Valkey 8.x as an alternative cache backend to Redis. This work is driven by Redis 7.2 end‑of‑support (EOS) and licensing changes, and gives merchants a fully supported Redis‑compatible cache option, while retaining compatibility with Redis 7.x.

_AC-14103_

#### Update Nginx version from 1.26 to 1.28

The Nginx version used in development and testing environments across all the currently supported version of Adobe Commerce is updated to 1.28, aligning with the latest stable  Nginx release available.
...
Adobe Commerce now supports the latest version of Apache ActiveMQ Artemis across release lines 2.4.9, 2.4.8, 2.4.7 and 2.4.6.
...
This update ensures Adobe Commerce remains future-proof and performant by supporting Valkey, giving merchants and developers a reliable alternative to Redis as it approaches end-of-life.
...
#### PHP 8.5 Compatibility for Adobe Commerce 2.4.9

Starting with Adobe Commerce 2.4.9, the platform is fully compatible with PHP 8.5, while retaining support for PHP 8.4 and allowing PHP 8.3 for upgrade-only scenarios. This work modernizes core code, dependencies, and tools so merchants can safely move to newer PHP versions ahead of PHP 8.4 end of support, maintaining PCI compliance and long-term platform health.

_AC-15615_

#### Remove PHP 8.2 support and ensure the compatibility with PHP 8.3 and latest PHP versions

Starting with Adobe Commerce 2.4.9, PHP 8.2 is no longer supported. The platform now targets PHP 8.3 and later PHP versions, with core code, dependencies, and tooling updated to run cleanly and reliably on PHP 8.4 and 8.5.

_AC-15758_

#### Replacing Zend_Cache with symfony/cache component

Starting with Adobe Commerce 2.4.9, the deprecated Zend_Cache component has been replaced by the Symfony Cache component. This update enhances cache performance, maintainability, and ensures long-term compatibility with PHP 8.x and future platform updates. Existing cache backends and cache management commands remain fully supported, with no changes required for current integrations.
...
#### Comprehensive multisite ACL

This update delivers a comprehensive hardening of multisite access control (ACL) in the Adobe Commerce admin, addressing both known and previously unknown permission issues in the admin_gws module.
The fix ensures that admin users with access to specific websites/stores cannot view or modify data that belongs to other websites/stores in a multisite deployment.
...
In Adobe Commerce 2.4.9, the built‑in DHL shipping integration now supports MyDHL RESTful APIs, while preserving compatibility with the legacy DHL Express XML API. Merchants can choose which DHL API to use in Admin, benefiting from modern REST capabilities without breaking existing XML‑based setups.
