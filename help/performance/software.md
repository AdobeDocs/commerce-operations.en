...
* Nginx and [PHP-FPM](https://www.php.net/manual/en/install.fpm.php)
...
Use a PHP version supported by the Adobe Commerce release that you are installing, as listed in the [system requirements](../installation/system-requirements.md). There are several factors to account for when configuring PHP to get maximum speed and efficiency on requests processing.
...
To get maximum speed out of [!DNL Commerce], you must activate the OpCache module and properly configure it. These settings are recommended for the module:
...
|--- | --- | --- | ---|
|Nginx | `worker_connections` | `/etc/nginx/nginx.conf` (Debian) | [Tuning NGINX for Performance](https://www.f5.com/company/blog/nginx/tuning-nginx)|
...
Recent [!DNL MySQL] releases include many performance improvements, and [!DNL MySQL] is generally distributed with good default settings. The most critical settings are:
...
[!DNL Commerce] distributes sample configuration files for supported [!DNL Varnish] versions that contain all recommended settings for performance. Among them the most critical in terms of performance are:
...
To store your assets in [!DNL Varnish], add the following VCL entries in your `default.vcl` file generated by [!DNL Commerce].
