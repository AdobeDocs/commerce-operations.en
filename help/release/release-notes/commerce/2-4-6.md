...
*  `Zend_Filter` has been replaced with `laminas/laminas-filter`. <!-- AC-5922-->
...
This release introduces the Adobe Commerce Extension metapackage v1.0.0, which automatically bundles select Adobe Commerce extensions with this core release. The versions of these extensions that are included in this metapackage are installed when `composer update` is run, simplifying the process of upgrading these extensions when upgrading to the latest core release. These extensions will still maintain independent release schedules.
...
*  Purchase Order GraphQL schema covers B2B company user Purchase Order Management functionality like create, edit, and delete Purchase Orders.
...
*  Masked quote IDs are now generated as expected for GraphQL queries that return information about purchase orders and associated quotes.
...
*  Adobe Commerce now displays an informative message when an `addProductsToCart` query fails when adding a product to the cart with insufficient permissions.
...
*  The `addBundleProductsToCart` query no longer add products to a quote if a required product option has no value. [GitHub-25676](https://github.com/magento/magento2/issues/25676)
...
*  Removed a fixed version PHP Code Sniffer dependency from the Magento 2 project's main `composer.json` file. [GitHub-35994](https://github.com/magento/magento2/issues/35994)

<!-- AC-7300-->

*  Removed unnecessary manipulation of `version` keys from within `sampledata:deploy`. Previously, running `bin/magento sampledata:deploy` produced an error message indicating that a `version` key was missing from composer.json. When the command is successfully executed, Adobe Commerce now confirms success and indicates the next steps to take. [GitHub-36554](https://github.com/magento/magento2/issues/36554)
...
*  Retrieving bundle options via the `BundleGraphQl` module no longer returns an error. Previously, the `\Magento\BundleGraphQl\Model\Resolver\Options\Collection` class generated an erroneous query to the database. Adobe Commerce returned this error: `"debugMessage": "Notice: Undefined offset: 16 in vendor/magento/module-bundle-graph-ql/Model/Resolver/Options/Collection.php on line 130"`.[GitHub-34991](https://github.com/magento/magento2/issues/34991)
...
*  Updated the project code of conduct (`CODE_OF_CONDUCT.md`). [GitHub-36639](https://github.com/magento/magento2/issues/36639)

<!-- AC-7500-->

*  The Google Adwords Code template has been refactored to improve readability and replace deprecated escaping functions with correct functions. [GitHub-36641](https://github.com/magento/magento2/issues/36641)
...
*  The `orders` output object of the `customer` query now contains a sort field. You can now sort orders by order number or creation date and in ascending or descending order.
...
*  The `Fixex.io` import service now works as expected. Currency rates are now imported successfully. [GitHub-36010](https://github.com/magento/partners-magento2ee/issues/36010)
...
*  The ISO 3166 regions for the Czech Republic have been added to the `directory_country_region` table. [GitHub-36346](https://github.com/magento/magento2/issues/36346)
...
*  You can now create a shipment for orders that contain an item quantity that is greater than zero and less than 1 when **[!UICONTROL Quantity uses decimals]** is enabled. Previously, when a product quantity of less than 1 was entered, the invoice was created correctly, but the shipment was not created. [GitHub-35971](https://github.com/magento/magento2/issues/35971)
