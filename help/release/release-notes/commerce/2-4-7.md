...
*  RabbitMQ 3.11. RabbitMQ v3.9 end-of-support date is July 2023. <!-- AC-7718 -->

*  Varnish 7.3

#### Other upgrades and replacements

*  Temando modules have been removed from the core code base. This feature was deprecated in Adobe Commerce 2.4.4.  <!-- AC-8130 -->

The following libraries have been updated to their latest versions

*  `moment.js` <!-- AC-7389 -->
*  `jQuery-UI` (v1.13.2)  <!-- AC-7775 -->
*  `jQuery` validation plugin (v1.19.5)  <!-- AC-8077 -->
...
*  The performance of Admin configuration save operations for large deployments (for example, 230 websites and more than 5000 store-level configurations) has been improved substantially. Merchants using the Admin to save store-level configurations for large deployments can potentially see an xxx improvement in processing speed. <!-- ACPT-826 -->
...
*  Added the following endpoints to provide REST API coverage of Shared Catalog assign and reset custom price actions:  <!-- LYNX-42 -->

`POST /V1/sharedCatalog/:sharedCatalogId/assignCustomPrices`
`POST /V1/sharedCatalog/:sharedCatalogId/resetCustomPrices`

*  Added the `quickorder_active` field to the `storeConfig` and `availableStores` queries. This field indicates whether the quick order feature is enabled. <!-- LYNX-61 -->
...
*  Display issues on the storefront product details page that were due to catalog permissions indexing have been resolved. Previously, Adobe Commerce cleared the index table first and then re-indexed during a full `catalogpermissions_product` or `catalogpermissions_category` indexing process. Catalog permissiosn did not work as expected on the storefront until the re-indexing completed.
