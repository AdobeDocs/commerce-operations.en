...
* The default behavior of the [isEmailAvailable](https://developer.adobe.com/commerce/webapi/graphql/schema/customer/queries/is-email-available/) GraphQL query and ([V1/customers/isEmailAvailable](https://adobe-commerce.redoc.ly/2.4.6-admin/tag/customersisEmailAvailable/#operation/PostV1CustomersIsEmailAvailable)) REST endpoint has changed. By default, the API now always returns `true`. Merchants can enable the original behavior, a value of `true` indicates that the email address is not in the database and the guest customer can use it to create an account. A value of `false` indicates that the email address is in the the database, and cannot be used to create an account. However, reverting to the previous default behavior can expose customer information to unauthenticated users.
