...
*  **Notes exchange on quote**. During the negotiation process, B2B buyers and sellers can improve communication by adding notes directly to a quote. Note creation and updates are included in the line item and quote history for transparency. See [Negotiate a Quote](https://experienceleague.adobe.com/docs/commerce-admin/b2b/quotes/quote-price-negotiation.html).

*  **Improved Quote Detail view**. Usability enhancements simplify access to detailed quote information, increasing transparency and streamlining the purchasing process. Sellers and buyers can now add items by SKU or from the product catalog, add or edit notes, apply line items discounts, and access product configuration directly from the quote detail page.
...
does not download the product image as expected. Instead, Adobe Commerce displays a blank screen when you click the link for a configurable product image. <!-- AC-8849 -->
