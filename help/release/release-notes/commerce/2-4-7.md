...
badgeCoreBeta: label="2.4.7-beta2" type="informative"
...
Adobe Commerce 2.4.7-beta2 includes over 200 quality fixes and enhancements. Core Composer dependencies and third-party libraries have been upgraded to the latest available versions. Support for Composer 2.5 has been added, while Composer 2.2.x remains supported. 
...
Adobe Commerce 2.4.7-beta2 introduces xxx security fixes and platform security improvements.

Previous betas of Adobe Commerce 2.4.7 introduced 13 security fixes and platform security improvements.
...
Previous beta releases of Adobe Commerce 2.4.7 introduced:
...
New upgrades and enhancements for 2.4.7-beta2

*   This release is packaged with Elasticsearch 8.x and is compatible with the latest patch version of Elasticsearch 8.10.x. Elasticsearch 7.17 has reached EOL and has been deprecated. AC-8767

*   The Commerce UPS XML API gateway has been migrated to the new Commerce UPS REST API to support updates that UPS is making to their [API security model](https://developer.ups.com/oauth-developer-guide?loc=en_US&sp_rid=NTA5MzQ1OTE2NjEyS0&sp_mid=72989914). (In short, UPS is implementing an OAuth 2.0 security model (bearer tokens) for all APIs.)  All previous Commerce UPS XML APIs have been removed from the Adobe Commerce 2.4.7 code base. AC-8918
...
This new integration  can be used to retrieve shipping rates and schedule deliveries and returns through the  USPS shipping service. The USPS Ground Advantage shipping method replaces these shipping methods, which were retied when the USPS Ground Advantage shipping method was released:
...
PHP App Server is now available for Cloud deployments. it is available on `p.sh` by default, but must be configured and started by a developer. See xxx

PHP App Server coverage has also been extended to cover Commerce B2B GraphQL APIs.  State management is now enabled by default for all the Adobe Commerce B2B GraphQL APIs. 

Previous betas of Adobe Commerce 2.4.7 introduced:

Commerce 2.4.7-beta1 introduced PHP Application Server for GraphQL APIs. This new PHP application server that is implemented on a Swoole PHP extension. This application server enables Adobe Commerce to maintain state between Commerce GraphQL API requests and eliminates the need for bootstrapping. By sharing application state among processes, API requests become significantly more efficient, and API response times potentially decrease by 50 - 60 milliseconds.
...
New features and fixes for 2.4.7-beta2
...
This release introduces support for the following payment methods:

ApplePay vaulted payments <!-- BUNDLE-3246 -->
GooglePay vaulted payments <!-- BUNDLE-3270 -->
PayPal credit vaulted payments <!-- BUNDLE-3272 -->
...
Merchants can enable and disable these payment methods 
...
* Refactored the way banner content is loaded to remove excessive database queries, which can improve cart load times.
...
* Fixed-discount amount that are applied at the cart level are now correct.[GitHub-37496](https://github.com/magento/magento2/issues/37496)
...
* Multi-select attributes are now saved as expected when Use Default Value in the store-view scope is selected. Previously, this checkbox was unselected was unchecked when the product was edited and the default value was saved.

<!-- ACP2E-1878 -->

* Admin users with website-limited access can now add Images to a product that falls within their scope. If the admin user doe snot have access rights to the product, Commerce displays an informative message. Previously, admin users with website-limited access could not add images to products to which they had access.
...
* Editing configuration of configurable products with skipping quantity per source at that time no longer cleans up assigned sources of simple products. Previously, skipping adding quantity per source resulted in disappearing sources from variations.
...
* The `bin/magento setup:config:set` command no longer crashes when an invalid database user name is included in the `app/etc/env.php` file. [GitHub-37409](https://github.com/magento/magento2/issues/37409)
...
* The `setGiftOptionsOnCart` mutation now removes gift wrapping from an order as expected.  The value of `gift_wrapping_id` is now unset at the cart level. Previously, the `gift_wrapping_id` was updated to it previous value after the order was placed even when this value was unset at the cart level.
...
* The `setGiftOptionsOnCart` query now successfully updates the database when removing a gift message from the cart. Previously, message details were not updated in the database, and the gift message was not removed from the cart.
...
* The `products` query now returns all relevant storefront pricing details when dynamic pricing is set to no. Previously, the price range for products was not displayed.[GitHub-35649](https://github.com/magento/magento2/issues/35649)
...
* The GraphQL product visibility filter for search results is now handled correctly. Previously, any filter provided was switched to the `Visibility: Catalog` setting. [GitHub-36591](https://github.com/magento/magento2/issues/36591)
...
* The **Card Security Code Does Not Match** configuration is now processed as expected during the Payflow Pro payment process. Previously, this evaluation process never completed, which blocked checkout.
...
* The advanced search price and SKU filters now work correctly with applied together In an advanced search. [GitHub-37581](https://github.com/magento/magento2/issues/37581)
...
* Test coverage has been added for these customer  `storeConfig` options: `required_character_classes_number` and `minimum_password_length`. [GitHub-37915](https://github.com/magento/magento2/issues/37915)
...
* Removing the breadcrumbs node by setting `remove=true` via the layout XML no longer results in page title removal on the category page. Previously, removing breadcrumbs this way removed the page title from this category page.
