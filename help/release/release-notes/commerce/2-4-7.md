...
This release introduces significant improvement in processing speed of configuration save operations for large deployments.

*  **State management has been enabled for all GQL APIs** (excluding B2B and service- related processes).  This release introduces a new PHP application server that is implemented on a Swoole PHP extension. This application server enables Adobe Commerce to maintain state between Commerce GraphQL API requests and eliminates the need for request bootstrapping. By sharing state among processes, API requests become significantly more efficient, and  API response times potentially decrease by 50 - 60 milliseconds.

The `Magento_ApplicationServer module` implements the PHP application server, which is available for  on-premises deployments only. Developers who want to implement this server must also install and configure Swoole and Nginx. See the xxx readme file for installation and configuration information.  <!-- ACPT-27 -->
