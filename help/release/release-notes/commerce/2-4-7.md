...
*   **JSON format now supported for the [Import API](https://developer.adobe.com/commerce/webapi/rest/modules/import/)**. Merchants can now import up to 100,000 records per minute into Adobe Commerce in JSON format. <!-- ACPT-943 -->
...
*   **Faster save operations of store-level configurations for deployments with many stores**. Saving configuration settings in deployments with more than 500 stores can be time-consuming. The new [Async Config](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/configuration.html?lang=en#asynchronous-configuration-save-2.4.7-beta1) module enables asynchronous configuration save operations by running a cron job that uses a consumer to process the save operation in a message queue. AsyncConfig is disabled by default.
...
The `ApplicationServer` module implements the PHP application server. Developers who want to implement this server in an on-premises deployment must also install and configure Swoole and Nginx. The module is available on `p.sh` by default, so developers of Cloud deployments do not need to Developers who want to implement this server must also install, but must be configured and started by a developer.

See [Application Server for GraphQL APIs](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/performance-best-practices/application-server.html) for on-premises deployment.

See xxx for information about enabling App Server in Cloud deployments. Merchants cannot start or stop App Server on Cloud as they would another service. Adobe Commerce must be redeployed to start or stop App Server, which requires site downtime.

This is a beta version of PHP App Server for GraphQL APIs. Deployments that include customizations and extensions may not be compatible with App Server. Merchants who enable App Server should be prepared to resolve potential incompatibilities between App Server and deployed customizations and extensions. <!-- ACPT-27 -->
...
* Resolver caching is now supported by: 
