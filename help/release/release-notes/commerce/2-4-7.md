---
title: Adobe Commerce 2.4.7-beta3 release notes
description: Learn about new features, enhancements, bug fixes, and known issues in the 2.4.7-beta3 Adobe Commerce release.
badgeCoreBeta: label="2.4.7-beta" type="informative"
exl-id: 6ceb1d6d-5076-49a1-925f-913aa3598c90
---

# Adobe Commerce 2.4.7-beta3 release notes
...
Adobe Commerce 2.4.7-beta3 introduces support for PHP 8.3. It introduces over 150 quality fixes and enhancements. Core Composer dependencies and third-party libraries have been upgraded to the latest available versions. This release includes significant performance improvments, including faster loading of product listing pages for complex products with many options. 
...
## Adobe Commerce 2.4.7-beta3 test environment

Adobe Commerce 2.4.7-beta3 has been tested against the following component versions:

* Chrome 118
* Composer 2.6 or the latest version compatible with PHP 8.3.
* Elasticsearch 8.11 or the latest available version 
* MariaDB 10.6
* MySQL 8.0
* Nginx 1.24
* OpenSearch 2.12
* PHP 8.3 and 8.2
* PHPUnit 10.x
* RabbitMQ 3.13 or the latest available version
* Redis 7.2
* Varnish 7.4.1 or the latest available version
...
Adobe Commerce 2.4.7-beta3 includes 19 security fixes and platform security improvements.
...
See [Adobe Security Bulletin](https://helpx.adobe.com/security/products/magento/apsb24-03.html) for the latest discussion of these fixed issues.
...
Adobe Commerce 2.4.7-beta3 includes the following platform upgrades:

**PHP 8.3 compatibility**. This release introduces support for PHP 8.3. Adobe Commerce 2.4.7-beta3. Commerce now supports both PHP 8.3 and 8.2. PHP 8.2 will be supported until its End of Service (EOS) date in December 2025. After December 2025, all merchants running 2.4.7 deployments should migrate to PHP 8.3. <!-- AC-8988 -->

Adobe Commerce 2.4.7-beta3 is still compatible with PHP 8.1 for upgrade purposes only. PHP 8.1 is not supported and not recommended. Adobe Commerce 2.4.7-beta3 core code, all bundled extensions, and all Adobe-owned extensions and SaaS services are compatible with PHP 8.3. <!-- AC-9668 -->

* **RabbitMQ 3.11 support**. This release is compatible with the latest version of RabbitMQ 3.11. Compatibility remains with RabbitMQ 3.9, which is supported through August 2023, but we recommended using Adobe Commerce 2.4.7-beta3 only with RabbitMQ 3.11.

* *Composer 2.5.x**. Compatibility with Composer 2.2.x remains. <!-- AC-9663 -->

* **Varnish cache 7.4 support**. This release is compatible with the latest version of Varnish Cache 7.4. Compatibility remains with the 6.0.x and 7.2.x versions, but we recommended using Adobe Commerce 2.4.7-beta3 only with Varnish Cache version 7.4 or version 6.0 LTS. <!-- AC-9662 -->
...
All JavaScript libraries and NPM dependencies in Adobe Commerce core have been updated to the latest available versions. All Laminas library dependencies have been updated to the latest version that are compatible with PHP 8.3. <!-- AC-9667 9665 -->
...
Commerce 2.4.7-beta3 includes the following enhancements to Commerce performance and scalability:

* Enterprise merchants can now configure up to **one million active, coupon-based cart price rules** in Adobe Commerce with no significant performance degradations of cart and checkout operations. <!— ACPT-1609 11—98>
...
This release introduces the Adobe Commerce Extension metapackage v2.0.0-beta3, which automatically bundles select Adobe Commerce extensions with this core release. The versions of these extensions that are included in this metapackage are installed when `composer update` is run, simplifying the process of upgrading these extensions when upgrading to the latest core release. These extensions maintain independent release schedules.

The Adobe Commerce Extension metapackage for Adobe Commerce 2.4.7-beta3 includes these extensions:
...
* Updates to REST and GraphQL
    ...
    * Enhanced web API coverage of the storefront features for shared catalog include new endpoints to assign and reset custom price actions: `POST /V1/sharedCatalog/:sharedCatalogId/assignTierPrices` and `POST /V1/sharedCatalog/:sharedCatalogId/resetTierPrices`.
...
* Device data for 3DS is now verified and displayed on the Braintree control panel. The `collectDeviceData` parameter is now passed as `true` in the `verifyCard` call for 3DS. <!-- BUNDLE-3327 -->

* The Admin Payments configuration page now includes a link to the release notes and Braintree extensions page. <!-- BUNDLE-3325 -->

* The Braintree SDK has been updated to the latest versions. <!-- BUNDLE-3323 -->
...
Adobe Commerce 2.4.7-beta3 introduces these new features:

* **Greater flexibility managing cart items**. The `clearCart` mutation now clears the contents of a specified shopping cart in a single action. <!-- LYNX-254 -->

* **Improvements in create cart mutations**. Added the `createGuestCart` mutation and deprecated `createEmptyCart`. Previously, if you used `createEmptyCart`, you could not determine whether the cart was for a guest or logged-in customer. <!-- LYNX-224 -->
...
**GraphQL enhancements introduced in previous beta releases**

Commerce 2.4.7-beta2 introduced support for resolver caching and expands support for custom customer attributes caching.
...
* GraphQL support has been added for order cancelation. The See [`cancelOrder`](https://developer.adobe.com/commerce/webapi/graphql/schema/orders/mutations/cancel-order/) mutation allows a customer to cancel an order, passing its identifier and a cancelation reason. <!-- LYNX-197 -->

* Added the `order_cancellation_enabled` and `order_cancellation_reasons.description` response fields to the `storeConfig` 
query to support user-initiated order cancelation requests. See [Query a store's order cancellation configuration](https://developer-stage.adobe.com/commerce/webapi/graphql/schema/store/queries/store-config/#query-a-stores-order-cancellation-configuration)
...
Commerce 2.4.7-beta1 added enhanced GraphQL caching abilities and support to the GraphQL schema for custom attributes, including:
...
**Multiple coupons per order support**

Merchants can now configure the maximum number of coupons that can be applied per order with the new **[!UICONTROL Maximum number of coupons per order]** configuration option. This value is set to 1 by default. REST support for this feature has been added to this release, and GraphQL support will be available soon. 

### Payments 

* GraphQL support has been added for core operations for all payment methods with the exception of Venmo. New GraphQL endpoints for payment services are described in [Checkout](https://developer.adobe.com/commerce/webapi/graphql/schema/checkout/). <!-- BUNDLE-3326 -->

* Credit card vaulting has been enhanced for all payment methods with the exception of Venmo. Shoppers can now vault, or save, their ACH payments and view or delete them from their account page. Merchants can enable and disable ACH vaulted payments from the Admin. <!-- BUNDLE-3324 -->
...
We have fixed hundreds of issues in the Adobe Commerce 2.4.7-beta3 core code. A subset of the fixed issues included in this release is described below. Fixes included in previous Commerce 2.4.7-betas are also described.
...
* The Admin grid actions dropdown menu can now hover as expected over a selected link for  Content > Pages (and shared catalogs in the B2B module) . Previously, this menu was mostly hidden, and admin users had to scroll to see options. [GitHub-36710](https://github.com/magento/magento2/issues/36710)
...
* Commerce now displays bundle products as expected on the Catalog page when **[!UICONTROL Use Flat Catalog Product]** is set to **yes**. [GitHub-33935](https://github.com/magento/magento2/issues/33935)
...
* The REST `V1/products/<sku>/media` endpoint can now process the simultaneous upload of multiple media images. Previously, Previously, while processing several requests simultaneously, inconsistent data was created, and Commerce threw an error when this data was saved into the database.
...
* Resolved subtotal rounding errors that occurred in transactions for products for which decimal quantity have been enabled. [GitHub-37817](https://github.com/magento/magento2/issues/37817)
...
* Corrected issues with the Admin implementation of the `getStoreLabel` attribute. [GitHub-37953](https://github.com/magento/magento2/issues/37953)
...
* REST `V1/products/<sku>` now tiggers an automated stock re-index  when updating a product's stock status. Previously, a stock re-index was not triggered for the product that was being updated.
...
* `AUTO_INCREMENT` of the `inventory_source_item` table is no longer increased with every `UPDATE` operation. Previously, each update increased the `AUTO_INCREMENT` of this table, which eventually caused the AUTO_INCREMENT value to be out of range when adding a new record to inventory_source_item table. As a result, admin users received the following error when they tried to create a new product from the Admin: `Numeric value out of range: 167 Out of range value for column 'source_item_id' at row 1, query was: INSERT INTO `inventory_source_item` (`source_code`, `sku`, `quantity`, `status`) VALUES`.
...
* URL rewrites are now generated based on store-level visibility configuration in multi-store deployments. Previously, URL rewrites were not generated for non-default stores when product visibility is set to not visible under the default store scope.
...
The following known issue will be fixed in Adobe Commerce 2.4.7 GA:
...
You can install Adobe Commerce 2.4.7-beta3 using [Composer](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).
