...
This release boosts the performance of large deployments with the introduction of the beta PHP application server for GraphQL APIs. It adds support to the GraphQL schema for customer EAV attributes and enhanced GraphQL caching abilities.
...
*  **Faster save operations of store-level configurations for deployments with many stores**. Saving configuration settings in deployments with more than 500 stores can be time-consuming. The new `Async Config` module enables asynchronous configuration save operations by running a `cron` job that uses a consumer to process the save operation in a message queue. `AsyncConfig` is **disabled** by default.  <!-- ACPT-831 -->
...
*  **An application server has been introduced for all GQL APIs** (excluding B2B- and service- related processes). This release introduces a new PHP application server that is implemented on a Swoole PHP extension. This application server enables Adobe Commerce to maintain  state between Commerce GraphQL API requests and eliminates the need for bootstrapping. By sharing application state among processes, API requests become significantly more efficient, and API response times potentially decrease by 50 - 60 milliseconds.

  The `Magento_ApplicationServer module` implements the PHP application server, which is available for  on-premises deployments only. Developers who want to implement this server must also install and configure Swoole and Nginx. See this [readme file](https://github.com/magento-performance/magento2ee/blob/application-server/app/code/Magento/ApplicationServer/README.md).  
  
  This beta version of this application server is available for on-premises deployments only; it does not support Cloud-based deployments. It does not yet support B2B GraphQL functionality. GraphQL requests may not work as expected inn-premises deployments when this version of the PHP application server is configured. <!-- ACPT-27 -->
...
The `adobe-ims-metapackage` v2.3.0 is bundled with Adobe Commerce 2.4.7-beta1. No functionality has changed in this release. <!-- CABPI-545-->

### B2B

This release introduces enhanced web API coverage of storefront shared catalog feature. B2B enhancements include: 
...
*  **Support for customer EAV attributes**. The GraphQL EAV attributes schema now supports extending customer attributes and customer address objects in the Admin and retrieving them using GraphQL.

    * attributes that can have multiple values (such as checkboxes or multi-select attributes)
    * custom attributes for customer and customer address <!-- LYNX-61 110-->

* **Enhanced GraphQL caching capabilities**. Caching capability has been added to these queries:

    * `storeConfig` <!-- B2B-2487 2258 —->
    * `currency` <!-- B2B-2255 -->
    * `countries` and `country` <!-- B2B-2256 -->
    * `availableStores` <!-- B2B-2257 -->
    * `symfony/process`
    * `customAttributeMetadata` <!-- B2B-2259 -->
...
*  **Ability to skip X-Magento-Cache-Id header inclusion for GraphQL requests**. Developers can now cache GQL query responses without factoring in the `X-Magento-Cache-Id` header. Avoiding the need for this header reduces the number of cache entries. Previously, this header was required to properly cache a GQL response. To avoid using this header for requests that do not require it, developers can add set the xxx flag in the GraphQL schema. <!-- B2B-2564 -->

*  Added the `quickorder_active` field to the `storeConfig` and `availableStores` queries. This field indicates whether the quick order feature is enabled. <!-- LYNX-58 -->
...
`customAttributeMetadataV2` <!-- LYNX-152 160-->

#### Deprecated queries

`attributesMetadata` <!-- LYNX-77-->
...
*  Running `bin/magento setup:config:set` without specifying the `--lock-db-prefix` parameter no longer erases the current value from the `env.php` file.

<!-- ACP2E-1505 -->

*  The `bin/magento setup:upgrade` command no longer fails with this type MySQL memory limit-related error: `PHP Fatal error:  Allowed memory size of 4294967296 bytes exhausted (tried to allocate 20480 bytes)`. Multi-select attribute migration has been optimized to consume less memory during `setup:upgrade`.
...
*  Shoppers can now place an order in a deployment that implements split database when Checkout Async is enabled. Previously, Adobe Commerce threw this error:  `An error occurred on the server. Please try to place the order again`.
...
*  `bin/magento setup:upgrade` now displays a more informative error message when a message queue topic does ot include a topic name. [GitHub-34246](https://github.com/magento/magento2/issues/34246)


<!-- https://github.com/magento/magento2/pull/36239 -->

*  `bin/magento setup:upgrade` now displays a more informative error when merged XML files are invalid. The error message now includes the filename. 
...
*  Performance issues when adding bundle products with optional options to the cart using the `addBundleProductsToCart` mutation have been resolved.

<!-- ACP2E-1254 -->

*  Out-of-stock bundle options are now included GraphQL responses when the **[!UICONTROL Display out of stock products]** setting is enabled.
...
*  Bundle product creation via the REST `pub/rest/default/V1/products` endpoint is now successful when catalog price scope is set to `website`. Previously, an integrity constraint violation occurred. [GitHub-35595](https://github.com/magento/magento2/issues/35595)
...
*  The plugin whose purpose is to add the configurable cache tag when a simple product associated with a configurable product is saved now works as expected. All relevant configurable cache tags are removed as expected when a simple product is updated by the REST API. Previously, not all prices were updated on the storefront, and chases had to be manually cleared. [GitHub-36726](https://github.com/magento/magento2/issues/36726)
...
*  The `GetCartDetails` query longer returns an error when a product in the specified cart is out of stock. Previously, shoppers using this query to make a purchase could not complete their purchase when an item was out of stock, and their checkout page was blank.

<!-- ACP2E-1459 -->

*  Guest API requests to `POST /rest/default/V1/guest-carts/<cartId>/shipping-information` no longer return a `cancel` status if the page reloads during login in a mobile environment. Previously, Adobe Commerce threw a 500 error and logged this exception: `TypeError: Argument 2 passed to Magento\CustomerCustomAttributes\Model\Plugin\ProcessCustomerShippingAddressCustomAttributes::beforeSaveAddressInformation() must be of the type string, null given`.

<!-- AC-7886 -->

*  Free shipping eligibility is now calculated as expected when the `Subtotal (Excl. Tax)` condition for cart rules is applied. [GitHub-36760](https://github.com/magento/magento2/issues/36760)
...
*  Server-side validation has been added to the process of creating product attributes through the API. You cannot enable the **[!UICONTROL Use in Layered Navigation Catalog]** field when the **[!UICONTROL Catalog Input Type for Store Owner]** field value is `Text Field`,  `Text Area`,  `Text Editor`, `Date`, or `Date and Time`. Previously, products were omitted from categories and search results.

<!-- ACP2E-1413 -->

*  The category page now re-loads with the correct pagination after you open the page, change the pagination, navigate to a product page, then return to the category page.  [GitHub-36563](https://github.com/magento/magento2/issues/36563)
...
*  Adobe Commerce now generates a 301 redirect for a product the category it belongs to has been moved to a new parent. [GitHub-37039](https://github.com/magento/magento2/issues/37039)

<!-- AC-2267 -->

*  Merchants can now create and save a product when website `id != 1` and single-store mode is enabled. Previously, the save action failed with this message: `SQLSTATE[23000]: Integrity constraint violation: 1452 Cannot add or update a child row: a foreign key constraint fails`.
...
*  Adobe Commerce now displays the Date of Birth, Tax/VAT Number, Gender, Telephone, Company, Fax fields on the edit customer page based on configuration settings. [GitHub-36196](https://github.com/magento/magento2/issues/36196)
...
*  `POST: order/notify-orders-are-ready-for-pickup` now sends an email alerting customers that an order is ready for pickup only when the order is ready. Email is now triggered only when the request has been fulfilled without an exception. 
...
*  Coupons are no longer flagged as used when payment fails for the order to which they've been applied. Queue messages are now processed in the order in which they are published when the consumer is started. Previously, the first message was pulled from the queue to simply check whether messages were queued, and was rejected afterward, which caused the first message to be processed last.

<!-- ACP2E-1627 -->

*  Adobe Commerce no longer throws an exception in the log file when user tries to print shipping label. Previously, if an extension modifies response headers containing an attachment, the system threw an exception in the log file.
...
*  The `getProductFiltersByCategory` query now returns correct labels. [GitHub-29635](https://github.com/magento/magento2/issues/29635)
...
*  Corrected an error in which the method that sets the current store in a GraphQL mutation executed before the method that does validation. [GitHub-31336](https://github.com/magento/magento2/issues/31336)
...
*  Attribute Options provided by the data source model and installed using the setup data patch (typically from third party modules) can now be used with customer segment rules. Previously, this combination did not return the list of customer that matched the configured segment.
...
*  Adobe Commerce no longer throws an SQL error when you create a schedule change for product while saving another product.  Previously, Adobe Commerce displayed this error:  `The product with the SKU "%1" couldn't be added to the current update`. 
...
*  The `tax class` value for newly created simple products is now set to the parent product's value when creating new variations of an existing configurable product in Store View scope. Previously, this value was set to none. As a result, no tax was added to these products in the cart, and and customers were undercharged tax. [GitHub-37180](https://github.com/magento/magento2/issues/37180)
...
*  Reverted a fix included in Adobe Commerce 2.4.6 that results in problems running unit tests for this release. [GitHub-37289](https://github.com/magento/magento2/issues/37289)

<!-- AC-8018 -->

*  Running `vendor/bin/phpunit -c dev/tests/unit/phpunit.xml.dist` no longer results in many test failures. [GitHub-34733](https://github.com/magento/magento2/issues/34733)
...
*  Clicking on a video thumbnail in the product gallery fullscreen mode now displays the video as expected. Previously, the video was not displayed, and the browser console displayed a JavaScript error. [GitHub-36456](https://github.com/magento/magento2/issues/36456)
...
*  You can now open an order from the Admin with the status `Received` when `AsyncOrder` is not enabled. Previously, you could not open the order detail page from the Admin when the order status was changed by REST API.

<!-- ACP2E-1526 -->

*  Adobe Commerce now successfully returns the `content` attribute (base64 image code) in `rest/V1/products/sku/media` product media API responses. The `content` attribute did not previously appear in the response.
