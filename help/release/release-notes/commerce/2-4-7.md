...
This release boosts the performance of large deployments with the introduction of a PHP application server for GraphQL APIs. It adds support to the GraphQL schema for custom scalar types and customer EAV attributes.
...
*  **Support for custom scalar types has been added to GraphQL schemas**. Developers can now implement custom scalar types to express values that require validation, including HTML, email, and URLs. Custom scalar types can be used for more advanced cases including EAV attributes, which can be expressed as a single scalar value or an array of scalars. [GitHub-36877](https://github.com/magento/magento2/issues/36877) <!-- AC-7976 -->
...
*  **Support for customer EAV attributes**. The GraphQL EAV attributes schema now supports extending customer attributes and customer address objects in the Admin and retrieving them using GraphQL.
    ...
    * custom attributes for customer and customer address <!-- LYNX-61 -->

*  **Enhanced caching capability**. Caching capability has been added to these queries:
...
*  The plugin whose purpose is to add the configurable cache tag when a simple product associated with a configurable product is saved now works as expected. [GitHub-36726](https://github.com/magento/magento2/issues/36726)
...
*  `routes` queries that use fragments now return category information as expected. Previously, an internal server error occurred on the category page.

<!-- ACP2E-1772 -->

*  Issues with the `cataloginventory_stock` partial reindexing process have been resolved, and indexer performance has improved. Previously, stock and product categories were not accurately updated.
...
*  Removed a `lib/Framework` autoloader line from the `composer.json` file. Removing this line reduces the amount of time needed to autoload framework files. [GitHub-34031](https://github.com/magento/magento2/issues/34031)
...
<!-- https://github.com/magento/magento2/pull/36436 -->

*  Transaction names have been added to New Relic GraphQL transactions.
