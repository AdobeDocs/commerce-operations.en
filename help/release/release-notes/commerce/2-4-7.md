...
The `ApplicationServer` module implements the PHP application server. Developers who want to implement this server in an on-premises deployment must also install and configure Swoole and Nginx. Cloud devekopers do not need to download these components since `p.sh` already includes the Nginx and OpenSwoole. Cloud developers  still need to enable App Server as described here. Merchants enabling Application Server in Cloud deployments cannot start or stop Application Server on Cloud as they would another service. Adobe Commerce must be redeployed to start or stop Application Server, which requires site downtime.

See [Application Server for GraphQL APIs](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/performance-best-practices/application-server.html) for both on-premises and Cloud deployment.

This is a beta version of PHP Application Server for GraphQL APIs. Deployments that include customizations and extensions may not be compatible with App Server. Merchants who enable App Server should be prepared to resolve potential incompatibilities between App Server and deployed customizations and extensions. <!-- ACPT-27 -->
