---
title: Magento Open Source 2.4.7-beta1 release notes
...
*  Shoppers can now add more products to the shopping cart than the amount specified in the **[!UICONTROL Minimum Qty Allowed in Shopping Cart]** setting. Previously, Magento Open Source threw an error when a shopper added additional products to the cart after first adding the minimum quantity. [GitHub-33955](https://github.com/magento/magento2/issues/33955)
...
*  Magento Open Source no longer throws an exception in the log file when a user tries to print shipping label. Previously, if an extension modifies response headers that contain an attachment, the system threw an exception in the log file.
...
*  Reverted a fix included in Magento Open Source 2.4.6 that resulted in problems running unit tests for this release. [GitHub-37289](https://github.com/magento/magento2/issues/37289)
...
*  The final price for grouped products on the home page in the Recently Viewed widget now includes tax when expected. [GitHub-35881](https://github.com/magento/magento2/issues/35881)
...
**Issue**:  Configuration values that are set by environment variables (`$_ENV`) are not updated as expected when store view codes consist of capital letters. Scope code was always converted to lower case when configuration values were retrieved.  <!-- ACP2E-1512 -->
...
**Issue**: The product image link in the order confirmation, invoice, and credit memo emails, and Admin order details page does not download the product image as expected. Instead, Adobe Commerce displays a blank screen when you click the link for a configurable product image. <!-- AC-8849 -->
...
**Issue**: Shoppers cannot navigate to a non-default store from the home page in a multi-store deployment. Shoppers can navigate to a non-default store from other pages. <!-- AC-8843 -->

**Issue**: Invalid GraphQL queries do not return error-type variables as expected. <!-- AC-8623 -->

**Issue**: The **[!UICONTROL Download Sample File]** button on the Admin **[!UICONTROL System]** > **[!UICONTROL Data Transfer]** > **[!UICONTROL Import]** > **[!UICONTROL Entity Types]**  page does not work for any entity type. <!-- AC-8841 -->

**Issue**: Product URLs are not correctly updated with category name in URL rewrites after creation of a new store view in multi-store deployments. <!-- AC-8907 -->
