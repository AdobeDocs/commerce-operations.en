...
* **RabbitMQ 3.13 support**. This release is compatible with the latest version of RabbitMQ 3.13. Compatibility remains with RabbitMQ 3.11, which is supported through August 2024, but we recommended using Magento Open Source 2.4.7 only with RabbitMQ 3.13. <!-- AC-9659 --> 
...
* Shoppers can now log in to their store account and vault Braintree payment information from their Accounts page for ease of later payment. <!-- BUNDLE-3328 -->

* The Admin Order grid now includes a Disputes column. This column lists the most recent dispute associated with each transaction, that dispute's status, and its dispute ID. <!-- BUNDLE-3342 -->

* The new frictionless transaction feature streamlines the shopping experience by reducing the number of steps required to complete a Braintree transaction when 3D Secure login is enabled. Shoppers are now challenged for 3D Secure login only when the associated bank requests it. Previously, shoppers were challenged whenever they tried to complete a transaction with a Braintree credit card. <!-- BUNDLE-3344 -->
...
* Categories are now visible to shoppers in stores where cache is set to Fastly CDN (**[!UICONTROL Caching Application=Fastly CDN]**). This affected both guest shoppers and registered customers.
...
* Multiple clicks in the **Sales** > **Orders** page's mass action drop-down menu no longer generates multiple POST requests.  [GitHub-37997](https://github.com/magento/magento2/issues/37997)
...
* The image uploader now works as expected dynamic rows. Images are now uploaded correctly when sent as a multinational array. [GitHub-37957](https://github.com/magento/magento2/issues/37957)
