...
* **ACSD-53118** (for Adobe Commerce >=2.4.0 <2.4.7) - Fixes the issue where the Cart Price Rule is applied using coupon code while the product has an empty attribute.
* **ACSD-54324** (for Adobe Commerce >=2.4.5 <2.4.7) - Fixes the issue where GraphQL requisition_lists request does not consider pagination settings and returns all results.
