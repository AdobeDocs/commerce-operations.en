...
* **ACSD-49877** (for Adobe Commerce and Magento Open Source >=2.3.7 <2.4.7*) - Fixes the issue where video autoplay does not work on mobile Safari when the video is linked directly to a remote video file and not a streaming service.
...
* **ACSD-49129** (for Adobe Commerce and Magento Open Source >=2.4.2 <2.4.6*) - Fixes the issue where the "content" attribute (base64 image code) is not returned in `rest/V1/products/sku/media` product media API responses.
