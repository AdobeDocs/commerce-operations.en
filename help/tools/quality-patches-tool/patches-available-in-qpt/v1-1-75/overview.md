...
1. **[ACSD-68925](/help/tools/quality-patches-tool/patches-available-in-qpt/v1-1-75/acsd-68925.md)**: Fixes an issue where responses for GraphQL requests are now aligned with the GraphQL over HTTP specs. A 4XX response code is returned when the request can't be parsed, is unauthorized, or encounters a general problem if the request is parsed.
