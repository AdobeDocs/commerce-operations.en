...
1. **AC-10737**: Fixes an issue where the bin/magento setup:db:status command does not recognize the JSON data type.
1. **AC-10737**: Fixes an issue where the bin/magento setup:db:status command does not recognize the JSON data type.
...
1. **ACP2E-4603**: Fixes the issue where running the Catalog Permissions product reindex left existing permission index rows unchanged, causing updated category permission grants to not be reliably reflected on products.
1. **ACP2E-4706**: Fixes the issue where products not enabled in the Admin scope were skipped by the Target Rule indexer.
...
1. **ACP2E-4475**: Fixes the issue where the products listing page incorrectly filters and sorts out-of-stock bundle products by price when the "Display Out of Stock Products" option is enabled.
