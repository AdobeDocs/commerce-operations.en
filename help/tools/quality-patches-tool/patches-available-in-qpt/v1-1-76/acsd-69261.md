...
description: Apply the ACSD-69261 patch to fix the Adobe Commerce issue where a cart price rule coupon configured for single use per customer is being reused multiple times, due to incorrect handling of the `times_used` attribute in partial invoice and the remaining quantity is canceled scenarios.
feature: Checkout
